...
class FeatureExtractor:
    ...
    def extract(self, traffic_data: Dict[str, Any]) -> ExtractedFeatures:
        ...
        # Flag counts (estimated from protocol)
        ...
        # Port scan score: require many unique ports from single/many sources
        # Formula redesigned to require actual port scanning behavior:
        # - Need 15+ unique ports to even register
        # - Score grows with port count, dampened by multiple source IPs (suggests legitimate)
        dst_ports = features['unique_dst_ports']
        src_ips = features['unique_src_ips']
        dst_ips = features['unique_dst_ips']

        # Only score as port scan if 15+ ports hit (normal browsing rarely does this)
        if dst_ports >= 15:
            # Higher score if few source IPs (more likely a scan)
            # Lower score if many source IPs (likely distributed legitimate traffic)
            features['port_scan_score'] = min(
                (dst_ports / 50) * (1.0 / max(src_ips, 1) ** 0.3), 1.0
            )
        else:
